...
const HeaderDropdown = ({
  ...
  <Dropdown
    ...
    overlay={
      <Menu
        onClick={e =>
          handleHeaderClick(e, visualise, onEdit, openVisualisation, isActive)
        }
      >
            ...
            onClick={() => {
              if ("values" in editable) {
                confirm({
                  ...
                  content:
                    "Are you sure you want to discard changes made to the form?",
                  okText: "Yes, discard",
                  okType: "danger",
                  cancelText: "Cancel",
                  onOk() {
...
const renderFormField = (
  ...
  const isEditable =
    [record[primary], "_all"].includes(editable.target) &&
    "field" in editable &&
    editable.field === field.name;
      ...
      <div className="editable-field">
        <EditableField
          ...
          onChange={e =>
            onEdit({
              ...editable,
              values: { ...editable.values, [record[primary]]: e }
            })
          }
          onOk={confirmEdit}
    ...
    return (
      <div className="form-field">
        ...
        <Tooltip
          title={
            !isActive &&
            "This item cannot be edited as the form it belongs to is no longer active"
          }
        >
          <Button
            className="button"
            ...
            onClick={e => {
              if (record[primary]) {
                onEdit({
                  stepIndex,
                  field: field.name,
                  target: record[primary]
                });
              } else {
                message.warning(
                  `This form field cannot be edited as the matching field (${primary}) for this record is empty.`
                );
              }
            }}
          />
...
const FormColumns = ({
  ...
  let isActive = true;
  if (step.activeFrom && !moment().isAfter(step.activeFrom))
    isActive = false;
  if (step.activeTo && !moment().isBefore(step.activeTo))
    isActive = false;
  ...
  step.fields.forEach(field => {
    ...
    const isPrimary = step.primary === field;
    ...
    columns.push({
      ...
      render: (text, record) => {
        return renderFormField(
          ...
          step.primary,
          field,
          _.get(editable, `values[${record[step.primary]}]`, null) !==
            null && _.get(editable, "field") === field.name
            ? editable.values[record[step.primary]]
            : text,
          record,
          editable,
          onEdit,
