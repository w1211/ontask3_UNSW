...
class ScheduleEmailTaskView(APIView):
    def post(self, request, format=None):
        #without authentication
        authentication_classes = ()
        permission_classes = ()
        #create an email sending Scheduler
        task_name = "email_sending_task_" + str(request.data['workflowId'])
        #pass in workflowId as one workflow has up to one email scheduler
        arguments= '{"workflow_id":"' + request.data['workflowId'] + '"}'

        schedule, _ = CrontabSchedule.objects.get_or_create(
            minute = '*',
            hour = int(request.data['schedule']['frequency'])*24,
            day_of_week = '*',
            day_of_month = '*',
            month_of_year = '*',
        )
        ...
        periodic_task = PeriodicTask.objects.create(
           crontab=schedule,
           name=task_name,
           task='ontask.tasks.schedule_email_sending',
           kwargs=arguments
        )
