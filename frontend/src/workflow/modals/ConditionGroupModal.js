...
import { Modal, Form, Alert, Row, Tree, Select, Button, Input, Cascader, Icon } from 'antd';
...
class ConditionGroupModal extends React.Component {
  ...
  handleOk = () => {
    const { form, workflow, conditionGroup } = this.props;
    form.validateFields((err, values) => {
      if (err) return;
      if (conditionGroup) {
        this.boundActionCreators.updateConditionGroup(workflow.id, conditionGroup, values);
      } else {
        this.boundActionCreators.createConditionGroup(workflow.id, values);
      };
    });
  };

  render() {
    const { conditionGroupModalVisible, form, formState, modalLoading, modalError, workflow, conditionGroup } = this.props;
    ...
    if ('conditions' in formErrors) {
      ...
      formErrors.forEach(condition => {
        ...
        condition.formulas.forEach(formula => { if (Object.keys(formula).length > 0) hasError = true });
      })
    };
    ...
    return (
      <Modal
        ...
        title={conditionGroup ? 'Edit condition group' : 'Create condition group'}
  ...
  return (
    <div style={{ display:'flex' }}> 
      ...
      */}
      ...
      {conditionCount > 1 && (
        <Button 
          disabled={conditionCount < 2} size="small" type="danger" icon="delete" shape="circle" style={{ marginLeft:'3px' }}
...
const mapStateToProps = (state) => {
  const { 
    conditionGroupModalVisible, workflow, formState, conditionGroup, modalLoading, modalError
  } = state.workflow;
  ...
  Form.create({
    ...
    mapPropsToFields(props) {
      ...
      if (formState) {
        ...
        formState.conditions && formState.conditions.forEach((condition, i) => {
          ...
          condition.formulas && condition.formulas.forEach((formula, j) => {
            fields[`conditions[${i}].formulas[${j}].fieldOperator`] = Form.createFormField(
              formState.conditions[i].formulas[j].fieldOperator ? formState.conditions[i].formulas[j].fieldOperator : {}
            );

            fields[`conditions[${i}].formulas[${j}].comparator`] = Form.createFormField(
              formState.conditions[i].formulas[j].comparator ? formState.conditions[i].formulas[j].comparator : {}
            );
          });
        });
