...
class AAFAuthRouter(APIView):
    ...
    def post(self, request, format=None):
        ...
        if user == "NOT_AUTHORIZED":
            response =  Response({"error": "Not Authorized"}, status=HTTP_403_FORBIDDEN)
        elif user == "EXPIRED_SIGNATURE":
            response = Response({"error": "Expired Signature"}, status=HTTP_403_FORBIDDEN)
        elif not user:
            response = Response({"error": "Login failed"}, status=HTTP_401_UNAUTHORIZED)
