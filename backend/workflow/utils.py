...
def validate_condition_group(workflow, condition_group):
    data = evaluate_filter(
        workflow["datalab"], workflow["filter"] if "filter" in workflow else None
    )
...
def check_condition_match(match, all_conditions_passed, item):
    ...
    if condition not in all_conditions_passed:
        raise ValidationError(
            "The condition '{0}' does not exist in any condition group for this workflow".format(
                condition
            )
        )
...
def populate_content(workflow, content=None, zid=None):
    ...
    for item in filtered_data:
        ...
        item_content = re.sub(
            r"{% .*? (.*?) %}(.*?)({% endif %}|(?={% .*? %}))",
            lambda match: check_condition_match(match, all_conditions_passed, item),
            content,
        )
        # Populate the field tags
        item_content = re.sub(
            r"{{ (.*?) }}", lambda match: populate_field(match, item), item_content
        )
