...
class DatasourceViewSet(viewsets.ModelViewSet):
    ...
    def perform_update(self, serializer):
        ...
        if datasource['name'] != serializer.instance['name']:
            diff['name'] = {'from': datasource['name'],
    ...
    def compare_matched_fields(self, request):
        ...
        matching_datasource = Datasource.objects.get(
            id=matching_field['datasource'])
        matching_fields = set([record[matching_field['field']]
                               for record in matching_datasource['data']])

        primary_key = self.request.data['primaryKey']
        primary_datasource = Datasource.objects.get(
            id=primary_key['datasource'])
        primary_keys = set([record[primary_key['field']]
                            for record in primary_datasource['data']])
    ...
    def get_sheetnames(self, request):
        ...
        if 'file' in self.request.data:
            ...
            try:
                ...
                obj = s3.Object(
                    request.data["bucket"], request.data["fileName"])
                file = obj.get()['Body']
            except:
